...
const CheckJobPage = () => {
  ...
  const [resumeFileForCompanies, setResumeFileForCompanies] = useState(null);
  ...
  const handleFileChangeForCompanies = (event) => {
    const file = event.target.files[0];
    setResumeFileForCompanies(file || null);
    if (file) {
      setErrorCompanies('');
  ...
  const handleGetCompanyRecommendations = async () => {
    if (!resumeFileForCompanies) {
      setErrorCompanies("Please upload a resume file to get company recommendations.");
      return;
    }
    ...
    const formData = new FormData();
    formData.append('resume_file', resumeFileForCompanies);

    try {
      const response = await fetch(`${API_BASE_URL}/api/company_recommendations`, {
        method: 'POST',
        body: formData,
      });
      ...
      setCompanyRecommendations(responseData.recommendations);
    } catch (err) {
      console.error("Error getting company recommendations:", err);
      setErrorCompanies(err.message || "Failed to get company recommendations from the server.");
    } finally {
  ...
  return (
    <div className="check-job-page-wrapper"> {/* Added a wrapper for better structure */}
      ...
      <div className="page-content-flex"> {/* For side-by-side layout if desired */}
            ...
            <div className="generated-resume-section">
              ...
              <button 
                onClick={() => {
                  ...
                  a.href = url;
                  a.download = `tailored-resume-${jobDetails?.title || 'resume'}.txt`;
        ...
        <div className="feature-section company-recommendations-section">
          <div className="feature-header">
            <h2>Recommended Companies Based on Your Skills</h2>
          </div>

          <div className="resume-upload-section">
            <h4>Upload Resume for Company Recommendations</h4>
            <input type="file" onChange={handleFileChangeForCompanies} className="file-input" disabled={isLoadingCompanies}/>
            {resumeFileForCompanies && <p className="file-name">Selected: {resumeFileForCompanies.name}</p>}
          </div>

          <button onClick={handleGetCompanyRecommendations} disabled={isLoadingCompanies || !resumeFileForCompanies} className="button get-companies-button">
            {isLoadingCompanies ? 'Analyzing...' : 'Get Company Recommendations'}
          </button>

          {extractedSkills.length > 0 && (
            <div className="extracted-skills-section">
              <h4>Skills Found in Your Resume ({extractedSkills.length}):</h4>
              ...
              {companyRecommendations.map((company, index) => (
                <div key={index} className="company-card">
                  <div className="company-header">
                    <h5>{company.name}</h5>
                    <span className="match-percentage">{company.match_percentage}% Match</span>
                  </div>
                  <p className="company-description">{company.description}</p>
                  <div className="company-roles">
                    <strong>Common Roles:</strong> {company.roles.join(', ')}
                  </div>
                  <div className="matching-skills">
                    <strong>Matching Skills:</strong> 
                    <div className="skills-list">
                      {company.matching_skills.map((skill, skillIndex) => (
                        <span key={skillIndex} className="skill-tag matched">{skill}</span>
                      ))}
                    </div>
                  </div>
                </div>
          ...
          {errorCompanies && !companyRecommendations.length && <p className="error-message">{errorCompanies}</p>}
        </div>
      </div>
    </div>
  );
