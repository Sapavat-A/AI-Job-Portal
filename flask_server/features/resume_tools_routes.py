...
from ..pages.ai_utils import parse_resume_with_llm, generate_tailored_section, reassemble_resume, extract_skills_from_resume, get_company_recommendations
...
@resume_tools_bp.route('/company_recommendations', methods=['POST'])
def company_recommendations_route_handler():
    ...
    try:
        ...
        skills = extract_skills_from_resume(resume_text, current_app.logger)
        
        if not skills:
            return jsonify({"error": "No skills found in resume"}), 400
        
        # Get company recommendations based on skills
        recommendations = get_company_recommendations(skills, current_app.logger)
        ...
        current_app.logger.error(f"Error in company_recommendations: {e}", exc_info=True)
        return jsonify({"error": f"Failed to generate company recommendations: {str(e)}"}), 500